...
Written for the 6 real pairings in `src/data/pairings.json`, not generic templates — tailored copy still reads better than a mail-merge template at this count. If the pairing set grows meaningfully further, revisit as templates.

Language check against CLAUDE.md / `.claude/agents/brand-content.md`: none of these use `proves, causes, drives, predicts, explains, responsible for, significant relationship, evidence that, linked to, associated with`. Safer language used throughout: `seems to`, `invites`, `as if`.

## `worldbank-population-total-annual` + `nasa-power-temp-annual`
...
## `nasa-power-temp-annual` + `worldbank-internet-users-pct-annual`
