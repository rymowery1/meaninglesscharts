...
## 2026-07-09/10 — Type system, divergent color scale, then a style guide reversal

- Built a full type-scale system (Epilogue sans/display, Baskerville serif) as CSS custom properties, applied across every page/component. See `docs/design/type-scale.md`.
- Applied a user-supplied 13-step divergent color scale to chart lines (series A/B) and dataset domain badges (one step per domain). See `docs/design/color-scale.md`.
- User then supplied a Bloomberg Markets–inspired data-visualization style guide (`~/Downloads/Data Visualization Style Guide.docx`, read via `textutil` since it's a binary .docx). The guide's own color rule directly contradicted what was just built: "Green-to-red scale is reserved for value movement or performance intensity. Do not use this scale for arbitrary categories." Two conflicts, both resolved with the user via explicit questions before implementing: domain badges are categorical, not performance data, so they were reverted to a neutral outlined pill; chart lines are two arbitrary unrelated series with no real "winner," so green/red gain-loss coloring was reverted to neutral black/gray.
- Also adopted the guide's `--interactive` token (`#1F5EFF` blue), replacing the original warm maroon (`#8a3b2e`) accent color site-wide (links, buttons, nav hover, reveal-panel trigger) — confirmed AA contrast (~5.1:1) against both white and as a button background.
- Scope was deliberately limited to colors, borders, and chart styling (Chart.js gridline/tooltip/axis config) — the guide is written for a financial terminal (tickers, candlesticks, sparklines, heatmaps, multi-security tables), none of which exist in this site's feature set, and none of which were built. Homepage's Display XL hero, the type scale, and spacing were explicitly left untouched.
- The 13-step scale (`--scale-00`–`--scale-12`) stays defined in `global.css`, unused by any component, in case a genuinely value-movement-based feature needs it later.

## 2026-07-10 — Expanded to 13 datasets, declined two proposed sources, added a normalization rule

- User asked to add more data sources. Given the choice between more keyless-source indicators, registering BLS/OpenAQ keys, or a brand-new source, chose more World Bank indicators (still keyless, no new setup).
- User then offered a Kaggle API key mid-task. Declined — `CLAUDE.md` explicitly lists "Random Kaggle datasets" under Avoid (unclear provenance/licensing on user-uploaded re-hosts). Flagged the conflict rather than silently using or silently ignoring the key; user confirmed skipping it.
- User also asked about `planecrashinfo.com`. Declined — three separate conflicts: scraping an HTML site (CLAUDE.md forbids scrapers), an unvetted small hobbyist source, and pairing fatality data risks violating "don't make sensitive human outcomes the punchline." User confirmed skipping it.
- Verified 5 candidate World Bank indicator codes live against the API before writing anything (per the project's own anti-hallucination rule — indicator codes are configuration, not data, but still shouldn't be guessed from training-data recall). One assumed code, `EN.ATM.CO2E.PC` (CO2 emissions), turned out archived/deleted; found and used the live replacement, `EN.GHG.CO2.PC.CE.AR5`, instead of proceeding on a stale assumption.
- Added: `worldbank-life-expectancy-annual`, `worldbank-co2-emissions-per-capita-annual`, `worldbank-unemployment-rate-annual`, `worldbank-electric-power-consumption-annual`, `worldbank-mobile-subscriptions-annual`. All fetched via the existing `scripts/fetch-data/worldbank.ts` (extended, not rewritten), `verificationStatus: approved`, `launchReady: true` (World Bank's license/methodology pattern was already established for the other 8).
- **Found a real normalization gap while checking `worldbank-mobile-subscriptions-annual`:** its earliest value in any weather-domain shared window (1981) is 0.0015 subscriptions per 100 people — mathematically valid to index (positive, no zero-crossing) but would inflate the scale to ~7,400,000% by 2024, a technically-passing pairing that would render as a visually broken chart. This wasn't an existing documented rule, so per `normalization-rules.md`'s own meta-rule (normalization-method changes need explicit approval, not a quiet code change), asked the user before implementing. Added a new `baseline-too-small` eligibility reason to `src/utils/normalize.ts` (rejects if baseline < 1% of the series' own peak in the shared window), documented in `normalization-rules.md`, tested in `tests/normalization.test.ts`. This is a generalizable fix, not a one-off patch for this one dataset.
